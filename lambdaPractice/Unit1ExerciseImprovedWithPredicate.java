...
//No more need to declare interface for lamdba use outof the box interface in this case Predicate which has test method we need
public class Unit1ExerciseImprovedWithPredicate {
    ...
    public static void printSome(List<Person> people, Predicate<Person> condition) {
        for(Person p:people) {
            if(condition.test(p)) {
                System.out.println(p);
            }
